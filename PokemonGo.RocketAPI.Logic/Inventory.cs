...
namespace PokemonGo.RocketAPI.Logic
{
    public class Inventory
    {
                ...
                .SelectMany(
                    p =>
                        p.OrderByDescending(x => x.Cp)
                         .ThenBy(n => n.StaminaMax)
                         .Skip(this._client.Settings.KeepMinDuplicatePokemon)
                         .ToList());
        }
        ...
        public async Task<IEnumerable<Item>> GetItemsToRecycle(ISettings settings)
        {
            ...
            return myItems
                .Where(x => settings.ItemsToKeep.Any(f => f.Key == (ItemId) x.Item_ && x.Count > f.Value))
                .Select(
                    x =>
                        new Item
                        {
                            Item_ = x.Item_,
                            Count = x.Count - settings.ItemsToKeep.Single(f => f.Key == (ItemId) x.Item_).Value,
                            Unseen = x.Unseen
                        });
        }
        ...
        private async Task<GetInventoryResponse> getCachedInventory()
        {
            ...
            try
            {
                ...
                this._cachedInventory = await this._client.GetInventory();
