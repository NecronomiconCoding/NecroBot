        ...
        public static async Task Execute(ISession session)
        {
            ...
            if (session.LogicSettings.DumpPokemonStats)
            {
                ...
                foreach (var pokemon in allPokemonInBag)
                {
                    ...
                    Dumper.Dump(session, $"NAME: {pokemon.PokemonId.ToString().PadRight(16, ' ')}Lvl: { PokemonInfo.GetLevel(pokemon).ToString("00")}\t\tCP: { pokemon.Cp.ToString().PadRight(8, ' ')}\t\t IV: { PokemonInfo.CalculatePokemonPerfection(pokemon).ToString("0.00")}%\t\t\tMOVE1: { pokemon.Move1.ToString()}\t\t\tMOVE2: { pokemon.Move2.ToString()} \t\tCandy: {candy}", dumpFileName);
                }
