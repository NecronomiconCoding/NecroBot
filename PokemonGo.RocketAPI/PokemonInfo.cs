...
namespace PokemonGo.RocketAPI
{
    ...
    public static class PokemonInfo
    {
        ...
        {
            BaseStats baseStats = GetBaseStats(poke.PokemonId); 
            return (baseStats.BaseAttack + 15) * Math.Sqrt(baseStats.BaseDefense + 15) * Math.Sqrt(baseStats.BaseStamina + 15); //issue: is this suppose to return large value?
        }
        public static double CalculateMinCPMultiplier(PokemonData poke)
        {
            BaseStats baseStats = GetBaseStats(poke.PokemonId);
            return (baseStats.BaseAttack) * Math.Sqrt(baseStats.BaseDefense) * Math.Sqrt(baseStats.BaseStamina); //issue:  is this suppose to return large value?
        }
        public static double CalculateCPMultiplier(PokemonData poke)
        {
            BaseStats baseStats = GetBaseStats(poke.PokemonId);
            return (baseStats.BaseAttack + poke.IndividualAttack) * Math.Sqrt(baseStats.BaseDefense + poke.IndividualDefense) * Math.Sqrt(baseStats.BaseStamina + poke.IndividualStamina); //issue returning large value
        }
        ...
        {
            return Math.Max((int)Math.Floor((GetBaseStats(poke.PokemonId).BaseStamina) * CpM[(int)PokemonInfo.GetLevel(poke) * 2 - 2]), 10);
        }
        public static int CalculateMaxHP(PokemonData poke)
        {
            return Math.Max((int)Math.Floor((GetBaseStats(poke.PokemonId).BaseStamina + 15) * CpM[(int)PokemonInfo.GetLevel(poke) * 2 - 2]), 10);
        }

        public static double CalculateMinEstTotalStamina(PokemonData poke)
        {
            return (poke.Stamina) / CpM[(int)PokemonInfo.GetLevel(poke) * 2 - 2];
        }
        public static double CalculateEstTotalStamina(PokemonData poke)
        {
            return (poke.Stamina + 0.5) / CpM[(int)PokemonInfo.GetLevel(poke) * 2 - 2];
        ...
        {
            return (poke.Stamina + 1) / CpM[(int)PokemonInfo.GetLevel(poke) * 2 - 2];
        }
        ...
        {
            return (-1 + (poke.Cp + 0.5) / ((GetBaseStats(poke.PokemonId).BaseAttack + 7.5) * Math.Sqrt(GetBaseStats(poke.PokemonId).BaseDefense + 7.5) * Math.Sqrt(GetBaseStats(poke.PokemonId).BaseStamina + 7.5) * Math.Pow(CpM[(int)PokemonInfo.GetLevel(poke) * 2 - 2], 2) / 10)) * 100;
        }
        public static double CalculateDeltaSta(PokemonData poke)
        {
            return (-1 + (poke.Stamina + 0.5) / ((GetBaseStats(poke.PokemonId).BaseStamina + 7.5) * CpM[(int)PokemonInfo.GetLevel(poke) * 2 - 2])) * 100;
        }

        public static double CalculateDeltaBR(PokemonData poke)
        {
            return (-1 + (poke.Cp + 0.5) / ((GetBaseStats(poke.PokemonId).BaseAttack + 7.5) * Math.Sqrt(GetBaseStats(poke.PokemonId).BaseDefense + 7.5) * Math.Sqrt(CalculateEstTotalStamina(poke)) * Math.Pow(CpM[(int)PokemonInfo.GetLevel(poke) * 2 - 2], 2) / 10)) * 100;
        }
        ...
        public static double CalculateMinBRCP(PokemonData poke)
        {
            return Math.Max(Math.Max(Math.Floor(GetBaseStats(poke.PokemonId).BaseAttack * Math.Sqrt(GetBaseStats(poke.PokemonId).BaseDefense) * Math.Sqrt(CalculateMaxEstTotalStamina(poke)) * Math.Pow(CpM[(int)PokemonInfo.GetLevel(poke) * 2 - 2], 2) / 10), 10), CalculateMinCP(poke));

        }

        public static double CalculateMaxBRCP(PokemonData poke)
        {

            return Math.Max(Math.Max(Math.Floor((GetBaseStats(poke.PokemonId).BaseAttack + 15) * Math.Sqrt(GetBaseStats(poke.PokemonId).BaseDefense + 15) * Math.Sqrt(CalculateMaxEstTotalStamina(poke)) * Math.Pow(CpM[(int)PokemonInfo.GetLevel(poke) * 2 - 2], 2) / 10), 10), CalculateMaxCP(poke));

        }


        public static double CalculatePokemonBattleRating(PokemonData poke,double battleRatingIVPercentage=50)
        {
            //return (poke.Cp - CalculateMinBRCP(poke)) / (CalculateMaxBRCP(poke) - CalculateMinBRCP(poke)) * 100;
            var maxCPatTrainerLevel = 3000;
        ...
        public static int GetPokemonCP(PokemonData poke)
        {
            return poke.Cp;
        }
