    ...
    internal class Statistics
    {
        ...
        public static async Task<string> _getcurrentLevelInfos(Inventory inventory)
        {
            ...
            if (stat != null)
            {
                ...
                output = $"{stat.Level} (next level in {hours}h | {stat.Experience - stat.PrevLevelXp - GetXpDiff(stat.Level)}/{stat.NextLevelXp - stat.PrevLevelXp - GetXpDiff(stat.Level)} XP)";
        ...
        {
            return  $"{PlayerName} - Runtime {_getSessionRuntimeInTimeFormat()} - Lvl: {CurrentLevelInfos:0} | EXP/H: {TotalExperience/_getSessionRuntime():0} " +
                    $"| P/H: {TotalPokemons/_getSessionRuntime():0} | Stardust: {TotalStardust:0} | Transfered: {TotalPokemonsTransfered:0} | Items Recycled: {TotalItemsRemoved:0}";
        }
